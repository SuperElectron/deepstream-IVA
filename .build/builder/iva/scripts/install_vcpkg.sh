...
# Example usage: ./install_vcpkg.sh
...
cd /tmp && wget "https://github.com/microsoft/vcpkg/archive/refs/tags/${VCPKG_VER}.zip" && \
  ...
  mv /tmp/vcpkg-"${VCPKG_VER}" "${HOME}/vcpkg" && \
  rm -rf /tmp/vcpk*

export VCPKG_FORCE_SYSTEM_BINARIES=1
cd "${HOME}/vcpkg"; ./bootstrap-vcpkg.sh;
cd "${HOME}/vcpkg"; ./vcpkg integrate install;
echo "${NAME} Install 'Google Tests' with vcpkg "
cd "${HOME}/vcpkg"; ./vcpkg install gtest;
echo "${NAME} Install 'bshoshany-thread-pool' with vcpkg "
cd "${HOME}/vcpkg"; ./vcpkg install bshoshany-thread-pool
echo "${NAME} Install 'bshoshany-thread-pool' with vcpkg "
cd "${HOME}/vcpkg"; ./vcpkg install glog
echo "${NAME} Install 'TZ Date' with vcpkg "
cd "${HOME}/vcpkg"; ./vcpkg install date
